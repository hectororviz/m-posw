...
exports.TreasuryReportsService = void 0;
const common_1 = require("@nestjs/common");
const exceljs_1 = require("exceljs");
const prisma_service_1 = require("../common/prisma.service");
let TreasuryReportsService = class TreasuryReportsService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    ...
    async ledgerAccount(accountId, from, to) {
        const account = await this.prisma.ledgerAccount.findUnique({ where: { id: accountId } });
        if (!account)
            throw new common_1.BadRequestException('Cuenta no encontrada');
        const lines = await this.prisma.journalEntryLine.findMany({
            where: {
                accountId,
                entry: {
        ...
        const result = accounts.map((a) => {
            ...
            if (net > 0) {
                if (isDebitNature)
                    ...
                    creditBalance = Math.round(net * 100) / 100;
            }
            else if (net < 0) {
                if (isDebitNature)
                    creditBalance = Math.round(-net * 100) / 100;
    ...
    async availabilityBalances(asOf) {
    ...
    async exportLedgerBook(from, to) {
    ...
    async exportLedgerAccount(accountId, from, to) {
    ...
    async exportTrialBalance(from, to) {
    ...
    async exportIncomeStatement(from, to) {
    ...
    async exportAvailabilityBalances(asOf) {
    ...
    styleNumberColumns(ws, colIndices, startRow = 2) {
...
exports.TreasuryReportsService = TreasuryReportsService;
exports.TreasuryReportsService = TreasuryReportsService = __decorate([
