"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ModuleAccessGuard = void 0;
const common_1 = require("@nestjs/common");
const core_1 = require("@nestjs/core");
const module_access_decorator_1 = require("./module-access.decorator");
const user_permissions_service_1 = require("../users/user-permissions.service");
const ACCESS_LEVEL = {
    HIDDEN: 0,
    READ: 1,
    FULL: 2,
};
let ModuleAccessGuard = class ModuleAccessGuard {
    constructor(reflector, userPermissionsService) {
        this.reflector = reflector;
        this.userPermissionsService = userPermissionsService;
    }
    async canActivate(context) {
        const metadata = this.reflector.getAllAndOverride(module_access_decorator_1.MODULE_ACCESS_KEY, [context.getHandler(), context.getClass()]);
        if (!metadata) {
            return true;
        }
        const request = context.switchToHttp().getRequest();
        const user = request.user;
        if (!user) {
            return false;
        }
        const access = await this.userPermissionsService.resolveAccess(user.id, metadata.module);
        return ACCESS_LEVEL[access] >= ACCESS_LEVEL[metadata.minAccess];
    }
};
exports.ModuleAccessGuard = ModuleAccessGuard;
exports.ModuleAccessGuard = ModuleAccessGuard = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [core_1.Reflector,
        user_permissions_service_1.UserPermissionsService])
], ModuleAccessGuard);
