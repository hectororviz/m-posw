...
let TournamentsService = TournamentsService_1 = class TournamentsService {
    ...
    async remove(id) {
        ...
        if (tournament._count.players > 0) {
            throw new common_1.BadRequestException('No se puede eliminar el torneo porque tiene jugadores fichados');
        }
        return this.prisma.tournament.delete({ where: { id } });
    }
    async getPlayers(tournamentId, params) {
        const tournament = await this.prisma.tournament.findUnique({
        ...
        if (params.search) {
            ...
            players = players.filter((p) => p.firstName.toLowerCase().includes(s) ||
                p.lastName.toLowerCase().includes(s) ||
                p.dni.includes(s));
        }
        return players;
    }
    async getEligiblePlayers(tournamentId) {
        const tournament = await this.prisma.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                categories: {
        ...
        const otherTournamentMap = new Map();
        ...
        const result = allPlayers
            ...
            .map((player) => {
            const assignedCategoryId = this.computeCategory(player.birthDate, tournament.year, tournament.categories.map((tc) => tc.category));
            return {
                id: player.id,
    ...
    async ficharJugadores(tournamentId, playerIds) {
        const tournament = await this.prisma.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                categories: {
        ...
        if (categories.length === 0) {
            throw new common_1.BadRequestException('El torneo no tiene categorías habilitadas. Agregá categorías antes de fichar jugadores.');
        }
    ...
    async desficharJugador(tournamentId, playerId) {
        ...
        if (!tp) {
            throw new common_1.NotFoundException('El jugador no está fichado en este torneo');
        }
        return this.prisma.tournamentPlayer.delete({
            where: { id: tp.id },
        });
    }
    computeCategory(birthDate, tournamentYear, categories) {
