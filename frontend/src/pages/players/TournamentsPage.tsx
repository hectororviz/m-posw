...
export const TournamentsPage: React.FC = () => {
  ...
  const [form, setForm] = useState({ name: '', year: new Date().getFullYear().toString(), allowedSex: 'M' as 'M' | 'F' | 'X', birthYearMin: '', birthYearMax: '', categoryIds: [] as number[] });
  const [formError, setFormError] = useState<string | null>(null);

  const openCreate = () => { setEditingId(null); setForm({ name: '', year: new Date().getFullYear().toString(), allowedSex: 'M', birthYearMin: '', birthYearMax: '', categoryIds: [] }); setFormError(null); setModalOpen(true); };
  ...
  const openEdit = async (id: number) => {
    try {
      ...
      const t = res.data;
      setEditingId(id);
      setForm({ name: t.name, year: t.year.toString(), allowedSex: t.allowedSex, birthYearMin: t.birthYearMin?.toString() ?? '', birthYearMax: t.birthYearMax?.toString() ?? '', categoryIds: (t.categories ?? []).map((c) => c.id) });
      ...
      <div className="sales-table-wrapper">
        <div className="sales-table">
          <div className="sales-table-head">
            ...
            <span className="col-category" style={{ flex: '0 0 200px' }}>Categorías</span>
            <span className="col-num" style={{ flex: '0 0 80px' }}>Jugadores</span>
            <span className="col-action" style={{ flex: '0 0 200px', textAlign: 'right' }}></span>
          </div>
          {isLoading ? (
            ...
            (tournamentsData?.data ?? []).map((t) => (
              <div key={t.id} className="sales-table-row">
                ...
                <span className="col-category" style={{ flex: '0 0 200px', display: 'flex', flexWrap: 'wrap', gap: '0.2rem', fontSize: '0.75rem' }}>
                  {(t.categories ?? []).map((c) => <span key={c.id} style={{ background: 'var(--color-blue-bg)', color: 'var(--color-blue-text)', padding: '0.1rem 0.4rem', borderRadius: '4px' }}>{c.name}</span>)}
                </span>
                <span className="col-num" style={{ flex: '0 0 80px', textAlign: 'center' }}>{t.playerCount ?? 0}</span>
                <span className="col-action" style={{ flex: '0 0 200px', textAlign: 'right' }}>
                  <button className="btn-ghost" onClick={() => openEdit(t.id)} style={{ marginRight: '0.25rem' }}>Editar</button>
                  <button className="btn-ghost" onClick={() => setPlayersModalTournament(t)} style={{ marginRight: '0.25rem' }}>
                    <UserPlus size={14} /> Jugadores
                  </button>
                  <button className="btn-ghost" onClick={() => setDeleteConfirm(t)} style={{ color: 'var(--color-danger)' }}>Eliminar</button>
                </span>
              </div>
            ))
          )}
                ...
                <div className="settings-field">
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginTop: '0.35rem' }}>
                    ...
                    {(allCategories ?? []).length === 0 && (
                      <span style={{ color: 'var(--color-text-muted)' }}>No hay categorías configuradas</span>
                    )}
                  </div>
                </div>
              </div>
