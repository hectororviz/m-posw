...
- **Botón en acreedores**: desde la lista de acreedores, un clic envía el mensaje con el saldo y la antigüedad de la deuda.
- **Plantilla personalizable**: mensaje configurable desde la GUI con variables `{{nombre}}`, `{{saldo}}`, `{{dias}}` y `{{club}}` que se reemplazan automáticamente.
